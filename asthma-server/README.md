...
> 천식환자 공간 조성 가이드 서비스 — **서버**

야간 생체 데이터(Fitbit) + 실내 환경(MQTT) + 실외 공기질을 통합해 개인 민감도에 따라 동적으로 달라지는 공간 조성 가이드를 Discord로 전달합니다. 임베디드 MQTT 브로커와 Mock 토글 덕분에 **외부 하드웨어/API 키 없이 단독 실행**이 가능합니다.
...
- **자동 API 문서**: `swagger-jsdoc` + `swagger-ui-express`로 OpenAPI 3.0 스펙 자동 생성
...
└── src/
    ...
    ├── app.js                # Express 앱 구성 + Swagger 마운트
    ├── config/index.js       # 환경변수 · 임계치
    ├── docs/openapi.js       # OpenAPI 스펙 빌더 + 재사용 스키마
    ├── mqtt/
    ...
    ├── routes/               # REST 엔드포인트 (+ @openapi JSDoc)
    │   ├── auth.routes.js
    │   ├── data.routes.js
    │   └── guide.routes.js
    └── mocks/
...
상세 스펙은 **Swagger UI(`/docs`)** 에서 확인. 요약:
